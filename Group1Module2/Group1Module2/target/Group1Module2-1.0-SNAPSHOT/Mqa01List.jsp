...
<body>
  ...
  <div class="content">
    <div class="table-container">
      <br>
      <center><h2>Mqa List</h2>
      <button style="background-color: #00cc00; padding: 10px; border-radius: 3px; border: 0px">
                    <a href="<%=request.getContextPath()%>/Mqa01.jsp" style="display: inline-block; width: 100%; height: 100%; text-decoration: none; color: white;">
                    Add New Mqa Document
                    </a></button>
      </center>
                    <br>
                    <br>
      <table border="1" width="100%">
        ...
        <tbody>
          <c:forEach var="mqa" items="${mqaList}">
            <tr>
              <td><c:out value="${mqa.docid}" /></td>
              <td><c:out value="${mqa.doctype}" /></td>
              <td><a href="${pageContext.request.contextPath}/mqa/download?docid=${mqa.docid}"><c:out value="${mqa.filename}" /></a></td>
              <td><c:out value="${mqa.date}" /></td>
              <td><c:out value="${mqa.pic}" /></td>
              <td class="actions">
                <a href="${pageContext.request.contextPath}/mqa/edit?docid=${mqa.docid}" class="edit-button">Edit</a> 
                <form action="${pageContext.request.contextPath}/mqa/delete" method="post" class="delete-form">
                  <input type="hidden" name="docid" value="${mqa.docid}">
